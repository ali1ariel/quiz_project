...
export MIX_ENV=prod
export APP_DIR="/opt/quiz_project"
...
# Compile the project
echo "Compiling..."
if ! asdf exec mix compile; then
  echo "Compilation failed"
  exit 1
fi
check_timeout

# Build and digest static assets (Tailwind + esbuild + phx.digest)
echo "Building assets..."
if ! asdf exec mix assets.setup; then
  echo "assets.setup failed"
  exit 1
fi
if ! asdf exec mix assets.deploy; then
  echo "assets.deploy failed"
  exit 1
fi
check_timeout
...
# Verify release
if [ ! -f "_build/prod/rel/quiz_project/bin/quiz_project" ]; then
