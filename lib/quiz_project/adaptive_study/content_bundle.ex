defmodule QuizProject.AdaptiveStudy.ContentBundle do
  ...
  Usa `Ash.Seed` em vez das actions normais porque a cópia precisa preservar
  `id`, timestamps e o `curated_material_id` que aponta pra outro material
  exatamente como estão — é cópia, não criação, e as actions/validações são
  para entrada de usuário, não para restaurar um estado que já existiu.
  """
  ...
  defp delete_existing(material_ids) do
    # `on_delete: :delete` em chapters/blocks/node_blocks cuida do resto.
    StudyMaterial
  ...
  defp insert_manifest(manifest, user_id) do
    ...
    |> Enum.each(&Ash.Seed.seed!(StudyMaterial, from_json_map(StudyMaterial, &1)))

    Enum.each(manifest["chapters"], &Ash.Seed.seed!(Chapter, from_json_map(Chapter, &1)))
    Enum.each(manifest["blocks"], &Ash.Seed.seed!(Block, from_json_map(Block, &1)))
    Enum.each(manifest["node_blocks"], &Ash.Seed.seed!(NodeBlock, from_json_map(NodeBlock, &1)))
  end
